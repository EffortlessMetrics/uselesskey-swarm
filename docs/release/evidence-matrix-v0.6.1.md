...
| Bundle verifier | `cargo run -p uselesskey-cli -- verify-bundle --path <bundle>` | Exported bundle manifests and files can be regenerated and checked. | Pending RC run |
...
| Scanner-safe bundle default | `uselesskey-cli` | `scanner-safe` bundle generation, manifest verification, materialization receipts, audit-surface receipts, and no-blob proof agree. | `cargo run -p uselesskey-cli -- bundle --profile scanner-safe --out target/uselesskey-bundle`; `cargo run -p uselesskey-cli -- verify-bundle --path target/uselesskey-bundle`; `cargo xtask no-blob` | Pending RC run |
