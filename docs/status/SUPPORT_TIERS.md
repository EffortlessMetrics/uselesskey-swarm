...
| Downstream policy pack recipes | Stabilizing | `metadata-only-audit-packets` | `cargo test -p uselesskey-cli --all-features audit_bundle`; `cargo xtask no-blob` | `docs/how-to/use-downstream-policy-pack.md`; `examples/external/ci-recipes/README.md` | Supports copyable downstream CI recipe wiring and metadata-only audit packet generation; recipe execution proof remains in `docs/status/workflow-support.md`. Does not prove release readiness, scanner-policy approval, provider compatibility, or downstream verifier correctness. | `minor` |
