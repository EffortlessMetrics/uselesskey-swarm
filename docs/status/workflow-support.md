...
| Downstream policy pack recipes | stabilizing installed CLI workflow | `metadata-only-audit-packets` | `docs/how-to/use-downstream-policy-pack.md`, `examples/external/ci-recipes/README.md` | `cargo xtask external-adoption-smoke --path . --ci-recipes --format json`; `cargo xtask no-blob` | `target/external-adoption-smoke/report.json`, `target/external-adoption-smoke/report.md` | Proves copyable downstream CI recipe wiring and metadata-only audit packet generation, not release readiness, scanner-policy approval, provider compatibility, or downstream verifier correctness. |
