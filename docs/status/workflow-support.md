...
| --- | --- | --- | --- | --- | --- | --- |
| Scanner-safe bundle handoff | stable bundle workflow | `scanner-safe-fixtures` | `docs/how-to/generate-scanner-safe-k8s-secret.md`, `docs/how-to/downstream-fixture-policy.md` | `cargo xtask scanner-safe-reference --check`; `cargo xtask no-blob`; `cargo xtask external-adoption-smoke --path .` | `target/external-adoption-smoke/report.json`, `target/external-adoption-smoke/report.md` | Scanner-safe does not mean scanner evasion or permission to commit runtime material. |
...
| JWT/token negative validation | stable facade workflow | `oidc-jwks-contract-pack` | `docs/how-to/test-jwt-negative-validation.md`, `examples/external/rust-test-fixtures/README.md` | `cargo test --manifest-path examples/external/rust-test-fixtures/Cargo.toml`; `cargo xtask external-adoption-smoke --path . --library-examples`; `cargo xtask check-negative-fixtures` | `target/external-adoption-smoke/report.json`, `target/external-adoption-smoke/report.md` | Proves deterministic token-shaped positives/negatives, not production authorization or signature assurance. |
...
| Installed bundle audit | incubating installed CLI workflow | installed CLI audit surface | `docs/how-to/share-installed-bundle-audit.md`, `docs/how-to/use-uselesskey-in-downstream-ci.md` | `cargo xtask external-adoption-smoke --path .`; `cargo xtask adoption-regression --external` | `target/external-adoption-smoke/report.json`, `target/adoption-regression/adoption-regression.json` | Proves local bundle consistency and metadata classification, not repo public claims or release readiness. |
...
| incubating installed CLI workflow | Installed command surface that is proven locally but may still refine options before 1.0. |
