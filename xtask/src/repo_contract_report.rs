...
fn build_report(root: &Path) -> Result<Report> {
    ...
    Ok(Report {
        ...
        ready_work_items: summarize_work_items(&goal, "ready"),
...
fn render_markdown(report: &Report) -> String {
    ...
    out.push_str(&format!(
        "| `{}` | {} | `{}` | `{}` |\n\n",
        report.active_goal.id,
        escape_md(&report.active_goal.title),
        report.active_goal.status,
        report.active_goal.path
    ));
...
mod tests {
    ...
    fn repo_contract_report_writes_markdown_and_json() -> Result<()> {
        ...
        assert_eq!(report.active_goal.id, "test-goal");
