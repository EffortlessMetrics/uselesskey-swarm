...
mod tests {
    ...
    // The routed workflow dispatches to self-hosted runners by label; actionlint
    // rejects any label it has not been told about, so the config must list them.
    #[test]
    fn actionlint_config_declares_self_hosted_runner_labels() -> Result<()> {
        let config_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../.github/actionlint.yaml");
        let config = fs::read_to_string(&config_path)
            .with_context(|| format!("read {}", config_path.display()))?;

        // Exact per-line match, not `contains`: the substring `- rust` is also a
        // prefix of `- rust-medium`, so a substring check would still pass after
        // the standalone `rust` label was deleted, and actionlint would then
        // reject every routed workflow.
        let declared = config
            .lines()
...
"#,
    ...
    fn routed_rust_workflow_uses_org_runner_discovery_and_capacity_contract() -> Result<()> {
        ...
        for expected in [
            ...
            "RUN_FULL_GATE: ${{ inputs.run_full_gate || false }}",
            "emit \"main-full\" \"main_full_gate\" \"false\" \"false\"",
            "orgs/${ORG}/actions/runners?per_page=100",
            "PR and merge-group runs keep one active run plus one pending replacement slot",
            "Pushes to main cancel",
            ...
            "write_route_receipt \"$target\" \"$reason\" \"$error\" \"$fallback_allowed\"",
            "ROUTE_RUN_ID=\"${GITHUB_RUN_ID}\"",
            "ROUTE_RUN_ATTEMPT=\"${GITHUB_RUN_ATTEMPT}\"",
            "ROUTE_SHA=\"${GITHUB_SHA}\"",
            "ROUTE_REF=\"${GITHUB_REF}\"",
            "target/source-of-truth/proof-route.json",
            "\"run_id\": os.environ[\"ROUTE_RUN_ID\"]",
            "\"run_attempt\": os.environ[\"ROUTE_RUN_ATTEMPT\"]",
            "\"sha\": os.environ[\"ROUTE_SHA\"]",
            "\"ref\": os.environ[\"ROUTE_REF\"]",
            ...
            "\"surface\": \"rails_control_plane\"",
            "\"surface\": \"goal_manifest\"",
            "\"reason\": \"implementation or unknown surface; route to broad Rust proof\"",
            ...
            "name: main-full-gate-receipt",
            "Check workflow hygiene guard",
            "Check routed workflow contract",
